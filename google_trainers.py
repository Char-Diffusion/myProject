...
import google_datasets  # pylint: disable=unused-import
import google_types
import google_utils
...
def standard_train_step(
  ...
  model_apply = google_utils.make_model_apply(model, apply_key)
...
def initialize_params_and_optimizer(
  ...
  batch = google_utils.build_batch_from_info(dataset_info)
  ...
  logging.info('model has %d parameters.', google_utils.num_params(params))
...
class Trainer:
  ...
  def __init__(
      ...
      *,
      ...
      learning_rate_fn=google_utils.create_learning_rate_scheduler,
      random_seed = 42,
      disable_pmap = False,
  ...
  def num_params(self, unreplicate=True):
    ...
    return google_utils.num_params(
  ...
  def load_checkpoint(self, ckpt_dir, **kwargs):
    ...
    restored_state = google_utils.load_checkpoint(state, ckpt_dir, **kwargs)
